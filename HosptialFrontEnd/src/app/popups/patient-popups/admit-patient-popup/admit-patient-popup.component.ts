...
@Component({
  ...
  templateUrl: './admit-patient-popup.component.html',
  styleUrls: ['./admit-patient-popup.component.scss'],
  imports: [
    IonicModule,
    ReactiveFormsModule,
    NgForOf
...
export class AdmitPatientPopupComponent extends ModalHandler implements OnInit{
  ...
  async admitPatient() {
    ...
    await firstValueFrom(this.patientService.setClinicalData(this.patientId, {clinicalRecord: this.clinicalData.value}).pipe(catchError((error: HttpErrorResponse) => this.handleError(error))));
